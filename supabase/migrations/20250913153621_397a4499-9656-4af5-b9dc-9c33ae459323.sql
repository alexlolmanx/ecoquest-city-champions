...
-- 2. Update user_roles table to use enum (fix the conversion)
ALTER TABLE public.user_roles ALTER COLUMN role DROP DEFAULT;
ALTER TABLE public.user_roles ALTER COLUMN role TYPE public.app_role USING 
  CASE 
    WHEN role = 'user' THEN 'user'::public.app_role
...
-- Create admin user and profile with the specified credentials
DO $$
DECLARE
  admin_user_id uuid;
BEGIN
  -- First, try to create the auth user (this might already exist)
  -- We'll handle this programmatically in the frontend instead
  -- since we can't directly insert into auth.users via migration
  
  -- Create a placeholder for manual admin setup
  INSERT INTO public.profiles (user_id, username, points_balance, level)
  SELECT 
